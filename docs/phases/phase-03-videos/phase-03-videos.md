...
| Reconciliation | Dispatcher scans every 30 s with a 2-minute row lease; if published job is absent and video unfinished for >2 min, reset intent to pending and republish. Inspect failed jobs to set terminal `error`; never restart `ready` or older generation. |
...
Deliverables: `src/videos/` module/controllers/services/entities, versioned migration, worker bootstrap and Dockerfile, private MinIO/Redis/worker Compose services, typed config and `.env.example`, unit/integration/e2e tests with small fixtures, updated Swagger/OpenAPI and phase documents. `progress.md` records SI status, actual commands, failures and residual risks. This plan defines the contract; the completed implementation and final command results are audited in `progress.md`. F03-10 additionally proved that the worker waits for migrations on a clean database before reporting healthy.

...
import { CreateVideos1790179200000 } from './migrations/1790179200000-CreateVideos';
...
describe('Database migrations (integration)', () => {
  ...
  afterAll(async () => {
    // The second test undoes the last migration, leaving video tables missing.
  ...
  it('should revert the last migration and remove video tables', async () => {
    ...
    const result = await dataSource.query<{ table_name: string }[]>(
      `SELECT table_name FROM information_schema.tables
       WHERE table_schema = 'public'
         AND table_name = ANY($1::text[])`,
      [['videos', 'video_processing_outbox']],
    );
